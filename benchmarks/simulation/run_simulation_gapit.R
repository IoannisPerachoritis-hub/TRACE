#!/usr/bin/env Rscript
# Run GAPIT3 MLM on a single simulated phenotype.
# Usage: Rscript run_simulation_gapit.R <scenario_dir> [n_pcs]
...
if (length(args) < 1) {
  cat("Usage: Rscript run_simulation_gapit.R <scenario_dir> [n_pcs]\n")
  quit(status = 1)
}
...
# ── Run GAPIT MLM ─────────────────────────────────────────
out_dir <- file.path(scenario_dir, "gapit")
