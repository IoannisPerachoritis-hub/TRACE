#!/usr/bin/env Rscript
# ──────────────────────────────────────────────────────────────
# GAPIT3 MLM benchmark on pre-QC'd data from TRACE
...
if (length(args) < 1) {
  cat("Usage: Rscript run_gapit.R <run_name> [n_pcs]\n")
...
cat(sprintf("=== GAPIT3 MLM benchmark: %s (PCs=%d) ===\n", run_name, n_pcs))

# ── Paths ────────────────────────────────────────────────────
# Assume script is run from project root
base_dir <- getwd()
qc_dir <- file.path(base_dir, "benchmarks", "qc_data", run_name)
out_dir <- file.path(base_dir, "benchmarks", "results", "gapit", run_name)
...
if (n_dropped > 0) {
  ...
  # Subset genotype columns (first col is SampleID)
  geno_raw <- geno_raw[, c("SampleID", keep_snps), with = FALSE]
}

# ── Convert to GAPIT format ──────────────────────────────────
# GAPIT needs:
#   myY: data.frame with Taxa + trait columns
#   myGD: data.frame with Taxa + SNP columns (numeric genotypes)
#   myGM: data.frame with Name, Chromosome, Position
...
tryCatch({
  ...
  cat(sprintf("\nGAPIT MLM completed in %.1f seconds\n", elapsed))
  ...
  # GAPIT outputs: GAPIT.Association.GWAS_Results.MLM.<trait>(NYC).csv
  gwas_files <- list.files(out_dir, pattern = "GWAS_Results.*\\.csv$",
                            full.names = TRUE)
  # Prefer the main results file (not StdErr or Filter)
  ...
  if (length(gwas_files) > 0) {
    ...
    # GAPIT output columns: SNP, Chr, Pos, P.value, MAF, nobs, Effect, H&B.P.Value
    std <- data.frame(
      SNP = gwas$SNP,
      Chr = gwas$Chr,
      Pos = gwas$Pos,
    ...
    write.csv(std, file.path(out_dir, "gapit_results_standardized.csv"),
              row.names = FALSE)
